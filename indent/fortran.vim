...
setlocal indentkeys+==~function,=~subroutine,=~module,=~program
setlocal indentkeys+==~endfunction,=~endsubroutine,=~endmodule,=~endprogram
...
function FortranGetIndent(lnum)
  ...
  if !exists("b:fortran_indent_less") && !exists("g:fortran_indent_less")
    ...
    if prevstat =~? '^\s*\(submodule\|program\)\>'
